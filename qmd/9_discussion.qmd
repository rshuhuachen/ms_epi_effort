...
![Histogram dispersion ratio changing CpG sites](../plots/model_out/changing_histogram_dispersion_glmer_raw.png) ![QQ-plot changing CpG sites](../plots/model_out/qqplot_changing_qqplot_glmer_raw.png)
...
![QQ-plot changing CpG sites threshold method](../plots/model_out/qqplot_changing_qqplot_glmer_threshold.png) ![QQ-plot changing CpG sites 90 percentile](../plots/model_out/qqplot_changing_qqplot_glmer_90percentile.png)
...
![Histogram dispersion ratio sites associated with centrality](../plots/model_out/hist_dist_dispersion_ratio_raw.png) ![QQ-plot sites associated with centrality](../plots/model_out/qqplot_dist_raw.png)
...
I have to check with some of my colleagues, but because we expect a relationship between pre-lekking methylation and body condition, we want to 'block' this pathway when we try to understand the relationship between e.g. reproductive effort and methylation changes (we want to close the 'backdoor' following McElreath's 'Statistical rethinking'; see \[https://speakerdeck.com/rmcelreath/statistical-rethinking-2023-lecture-06\] or other summaries on 'the haunted DAG & the causal terror' chapter).

This would then be the case for all models (including those for fitness). I'm finding this really difficult though so I'm not sure what the most correct decision is.

Furthermore, when trying to think of this DAG I also realized that the relationship between reproductive effort, physiological changes and parasites is complex, and I wonder if the way I modelled things (separately) is the best approach. I would say yes, as long as we describe this complex relationship well (e.g. a male fights a lot, loses body weight, is more susceptible to parasite infection, gets 'ill', stops investing in reproduction but in immune response instead, reduced fighting rate). 
