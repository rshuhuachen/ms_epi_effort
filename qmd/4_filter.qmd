...
Our dataset consists of 118 samples that have been epi-genotyped using epiGBS2. The samples are part of a bigger dataset consisting of 450 epiGBS samples, and therefore have been sequenced across 10 libraries of 22 samples each (of which not all samples were used for the current analysis).
...
We also filter for CpG sites that are variable in at least X number of samples, where X is a threshold that can be defined within the function. We used a threshold of 0.3, meaning that CpG sites that are 0% methylated or 100% methylated in more than 70% (1-0.3*100) of the samples will be excluded. Although we exclude a large number of CpG sites in this filtering step, it is a consative step that we think is necessary as it will allow the models to compute more accurate estimates and will only include CpG sites with adequate variation in methylation %.
