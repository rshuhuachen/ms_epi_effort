...
One for annual mating success (AMS):
`glmmTMB(MS ~ scale(delta_meth) + age + scale(methperc_pre) + (1|site/id), family = "poisson", ziformula=~1)`

And one for survival:
`glmmTMB(surv ~ scale(delta_meth) + age + scale(methperc_pre) + (1|site/id), family = "binomial")`
...
![Raw data AMS top CpG](../plots/model_out/rawdata_plot_AMS_cpg_top.png)
