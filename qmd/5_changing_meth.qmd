...
After the initial quality checks, we now want to dig into our research question. First, we aim to identify CpG sites that significantly change in methylation level over the lekking season. To do so, we build a generalized linear mixed effect model per CpG site, using the time period (pre- or post-lekking) as a fixed effect.

Additionally, as there are (sometimes) multiple samples for each individual, we correct for the non-independence of individuals by including ID as a random effect, nested within lek. The response variables in these models are both the number of methylated and number of unmethylated C's, using the `cbind` function.
...
When excluding models with convergence warnings/errors, we retain 97% of models, 345,983 out of 354,765 CpG site models. Singularity warnings are being kept as this is a consequence of having a random effect of ID, which is required to correctly control for the non-independence of our data.
...
Next, we filter out the sites with an overdispersion ratio higher than the 95 percentile as there seems a bit of overdispersion.
...
But, we first have to calculate delta methylation (post-lekking methylation % minus pre-lekking methylation %) in order to get this average.
...
Interestingly, there's a lot of GO terms that are related to gene regulation, although only one term remains significant after correcting for multiple testing.
