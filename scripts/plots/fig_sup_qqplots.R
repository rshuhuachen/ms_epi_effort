...
ggplot(out_changing, aes(x = prepost_pval)) + geom_histogram(col = "black", fill = clr_grey) + 
  ...
  labs(x = expression(italic(p)~values), y = "Count")-> hist_pvals_changing

# qq plot 
qq_plot(out_changing$prepost_pval, title = "", CB=F, thinning = F) -> qqplot_changing

## ams
# histogram p-values raw
ggplot(out_ams, aes(x = ams_delta_meth_pval)) + geom_histogram(col = "black", fill = clr_grey) + 
  labs(x = expression(italic(p)~values), y = "Count")-> hist_pvals_ams

# qq plot 
qq_plot(out_ams$ams_delta_meth_pval, title = "", CB=F, thinning = F) -> qqplot_ams

## surv
# histogram p-values raw
ggplot(out_surv, aes(x = surv_delta_meth_pval)) + geom_histogram(col = "black", fill = clr_grey) + 
  labs(x = expression(italic(p)~values), y = "Count")-> hist_pvals_surv

# qq plot 
qq_plot(out_surv$surv_delta_meth_pval, title = "", CB=F, thinning = F) -> qqplot_surv
...
ggplot(out_attend, aes(x = parameter_pval)) + geom_histogram(col = "black", fill = clr_grey) + 
  labs(x = expression(italic(p)~values), y = "Count")-> hist_pvals_attend

# qq plot 
qq_plot(out_attend$parameter_pval, title = "", CB=F, thinning = F) -> qqplot_attend

## fight
# histogram p-values raw
ggplot(out_fight, aes(x = parameter_pval)) + geom_histogram(col = "black", fill = clr_grey) + 
  labs(x = expression(italic(p)~values), y = "Count")-> hist_pvals_fight

# qq plot 
qq_plot(out_fight$parameter_pval, title = "", CB=F, thinning = F) -> qqplot_fight

## dist
# histogram p-values raw
ggplot(out_dist, aes(x = parameter_pval)) + geom_histogram(col = "black", fill = clr_grey) + 
  labs(x = expression(italic(p)~values), y = "Count")-> hist_pvals_dist

# qq plot 
qq_plot(out_dist$parameter_pval, title = "", CB=F, thinning = F) -> qqplot_dist

plot_grid(hist_pvals_attend, qqplot_attend, 
          hist_pvals_fight, qqplot_fight,
          hist_pvals_dist, qqplot_dist,
          labels=c("a)", " ", "b)", " ", "c)", " "), align="hv", axis="lb", ncol=2, label_fontface = "plain", label_size = 22)-> qqs_2
...
ggplot(out_mass, aes(x = parameter_pval)) + geom_histogram(col = "black", fill = clr_grey) + 
  labs(x = expression(italic(p)~values), y = "Count")-> hist_pvals_mass

# qq plot 
qq_plot(out_mass$parameter_pval, title = "", CB=F, thinning = F) -> qqplot_mass

## microf
ggplot(out_microf, aes(x = parameter_pval)) + geom_histogram(col = "black", fill = clr_grey) + 
  labs(x = expression(italic(p)~values), y = "Count")-> hist_pvals_microf

# qq plot 
qq_plot(out_microf$parameter_pval, title = "", CB=F, thinning = F) -> qqplot_microf

## trypa
ggplot(out_trypa, aes(x = parameter_pval)) + geom_histogram(col = "black", fill = clr_grey) + 
  labs(x = expression(italic(p)~values), y = "Count")-> hist_pvals_trypa

# qq plot 
qq_plot(out_trypa$parameter_pval, title = "", CB=F, thinning = F) -> qqplot_trypa

# igg
ggplot(out_igg, aes(x = parameter_pval)) + geom_histogram(col = "black", fill = clr_grey) + 
  labs(x = expression(italic(p)~values), y = "Count")-> hist_pvals_igg

# qq plot 
qq_plot(out_igg$parameter_pval, title = "", CB=F, thinning = F) -> qqplot_igg

# hct
ggplot(out_hct, aes(x = parameter_pval)) + geom_histogram(col = "black", fill = clr_grey) + 
  labs(x = expression(italic(p)~values), y = "Count")-> hist_pvals_hct

# qq plot 
qq_plot(out_hct$parameter_pval, title = "", CB=F, thinning = F) -> qqplot_hct
...
plot_grid(hist_pvals_mass, qqplot_mass,
          ...
          labels=c("a)", " ", "b)", " ", "c)", " ", "d)", " ", "e)", " "), align="hv", axis="lb", ncol=2, label_fontface = "plain", label_size = 22)-> qqs_3
