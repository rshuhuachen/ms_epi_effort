...
pacman::p_load(rptR, future.apply, parallel)
...
rpt_nonchanging <- rpt(
  cbind(numC, cov) ~ prepost + (1|id) + (1|chr_pos),
  grname="chr_pos",
  data=prepost_long,
  datatype="Proportion",
  nboot=0,
...
data_subsets <- lapply(site_samples, function(sites) {
  prepost_long %>%
    ...
    dplyr::select(id, chr_pos, numC, prepost, cov, methperc)   # drop unused columns if any exist
})
...
run_one <- function(dat) {
  ...
  data.frame(
    repeatability = rpt_fit$R["chr_pos"]
  )
}
...
# also for ALL sites
cat("Running all sites...\n")
rpt_all <- run_sites_sequential(valid_non_changing, prepost_long)
...
save(rpt_all,     file = "results/repeatability/results_ALL_per_cpg.RData")
