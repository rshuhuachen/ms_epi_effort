...
function_model_ams <- function(df, pre = pre){tryCatch({
  ...
  if (pre == "control"){
    formula <- formula(paste0("delta_meth ~ MS + methperc_pre + (1|site) "))}
  
  if (pre == "no_control"){
    formula <- formula(paste0("delta_meth ~ MS + (1|site) "))}
...
delta_out_ams_raw <- parallel::mclapply(delta_meth_ls_ams, function_model_ams, pre="control",mc.cores=4)
...
ggplot(delta_out_ams, aes(x = dispersion.ratio)) + geom_histogram() + geom_vline(xintercept = 1., col = "red", linetype = "dotted", linewidth=1) +
...
ggplot(delta_out_ams, aes(x = parameter_pval)) + geom_histogram() + 
...
qqplot.pvalues(delta_out_ams$parameter_pval, col.abline = "red", col.CB = "gray80", CB=TRUE, CB.level = 0.95) 
dev.off()


#### FDR-correction ####

delta_out_ams$parameter_qval <- p.adjust(delta_out_ams$parameter_pval, method = "fdr", n = nrow(delta_out_ams))
