...
function_model_delta_pheno_norepeat <- function(df, parameter, pre){tryCatch({
  ...
  age_estimate <- summary$coefficients[3,1]
  age_se <- summary$coefficients[3,2]
  age_df <- summary$coefficients[3,3]
  age_tval <- summary$coefficients[3,4]
  age_pval <- summary$coefficients[3,5]
...
function_process_model <- function(list, dir_plots, dir_data, name_file, pretty_name, filter_disp){
  ...
  data$parameter_qval_holm <- p.adjust(data$parameter_pval, method = "holm", n = nrow(data))
...
function_lmer_nextyear <- function(df, parameter){tryCatch({
  chr_pos <- as.character(df[1,1])
  df <- as.data.frame(df)
  
  formula <- formula(paste0(parameter, " ~ scale(delta_meth) + age_cat + (1|site)"))
  ...
  #fixed effect
  deltameth_estimate <- summary$coefficients[2,1]
  deltameth_se <- summary$coefficients[2,2]
  deltameth_df <- summary$coefficients[2,3]
  deltameth_tval <- summary$coefficients[2,4]
  deltameth_pval <- summary$coefficients[2,5]
  ...
  return(data.frame(chr_pos=as.factor(chr_pos),
                    response = as.factor(parameter),
                    intercept = as.numeric(intercept),
                    icc_site = as.numeric(icc_site),
                    deltameth_estimate = as.numeric(deltameth_estimate),
                    deltameth_se = as.numeric(deltameth_se),
                    deltameth_df = as.numeric(deltameth_df),
                    deltameth_tval = as.numeric(deltameth_tval),
                    deltameth_pval = as.numeric(deltameth_pval),
                    age_estimate = as.numeric(age_estimate),
                    age_se = as.numeric(age_se),
                    age_df = as.numeric(age_df),
                    age_tval = as.numeric(age_tval),
...
function_process_model_nextyear <- function(list, dir_plots, dir_data, name_file, pretty_name, filter_disp){
  pacman::p_load(dplyr, cowplot)
  ...
  ggplot(data, aes(x = deltameth_pval))+ geom_histogram() + 
    scale_y_continuous(labels = scales::unit_format(unit = "K", scale = 1e-3)) + 
    labs(title = "Histogram p-values", subtitle= paste0("Raw model output ", data$parameter[1])) -> hist_pval
  ...
  # FDR correction
  data$deltameth_qval <- p.adjust(data$deltameth_pval, method = "fdr", n = nrow(data))
  data$deltameth_qval_holm <- p.adjust(data$deltameth_pval, method = "holm", n = nrow(data))
  
  save(data, file = paste0(dir_data, "/out_", name_file, ".RData"))
  
  # subset significant ones 
  
  sig <- subset(data, deltameth_qval < 0.05)
  
  # volcano plot
  
  source("scripts/plotting_theme.R")
  
  data$response <- gsub(as.character(data$response[1]), pretty_name, data$response)
  data <- data %>% mutate(sig = case_when(deltameth_qval < 0.05 ~ "sig", TRUE ~ "nonsig"))
  clrs <- viridisLite::viridis(6)
  
  ggplot(data, aes(x = deltameth_estimate, y = -log10(deltameth_qval))) + geom_point(size=4, alpha=0.5, aes(col = sig)) +
...
function_poisson_glmer_nextyear <- function(df, parameter){tryCatch({
  chr_pos <- as.character(df[1,1])
  df <- as.data.frame(df)
  
  formula <- formula(paste0(parameter, " ~ scale(delta_meth) + age_cat + (1|site)"))
  
  model <- tryCatch(glmer(formula, data=df, family = "poisson"),
                    warning = function(w) {
                      warning_message <<- conditionMessage(w)  # capture the warning message
                    },
                    error = function(w) {
  ...
  deltameth_estimate <- summary$coefficients[2,1]
  deltameth_se <- summary$coefficients[2,2]
  deltameth_tval <- summary$coefficients[2,3]
  deltameth_pval <- summary$coefficients[2,4]
  
  age_estimate <- summary$coefficients[3,1]
  age_se <- summary$coefficients[3,2]
  age_tval <- summary$coefficients[3,3]
  age_pval <- summary$coefficients[3,4]
...
function_process_glmer_model_nextyear <- function(list, dir_plots, dir_data, name_file, pretty_name, filter_disp){
  pacman::p_load(dplyr, cowplot)
